    ...
    fetch(`${apiEp}`, {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          getCajadata(urlApi, idItem, cajId); //const getCajadata = async (urlApi, idItem, cajId) //'cjuId', 'http://serviciofact.mercelab.com/cajausuario/','http://serviciofact.mercelab.com/cajausuario/usuario?idcaja='
...
const Asignacion = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`http://serviciofact.mercelab.com/caja/listar/${gecid}/${empid}`);
  ...
  const actualizarCaja = () => {
    ...
    fetch(`http://serviciofact.mercelab.com/caja/${idcaja}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const nuevaCaja = () => {
    ...
    fetch(`http://serviciofact.mercelab.com/caja/crear`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
              ...
              {cajasData.map((data) => (
                <div className="pb-2" key={data.cajId} id="div">
                  <div className="card flex justify-content-center" id={data.cajId}>
                    ...
                    onClick={() => {admCajauser.getCajadata('http://serviciofact.mercelab.com/cajausuario/usuario?idcaja=','cjuId',data.cajId);admCajaser.getCajadata('http://serviciofact.mercelab.com/cajaserie/serie?idcaja=','cjsId',data.cajId);admCajadoc.getCajadata('http://serviciofact.mercelab.com/cajadocumento/documento?idcaja=','cjdId',data.cajId) }}
                  ...
                  <Card title="Usuarios" color='blue'>
                    ...
                    <Toast ref={admCajauser.toast} />
                      <AutoComplete
                        ...
                        completeMethod={(event) => cajaUserB.buscarC(`http://serviciofact.mercelab.com/cajausuario/usuarionot?idcaja=`, event, 'usuNombrecompleto')}
                        onChange={(e) => admCajauser.setSelectedDato(e.value)}
                        dropdown className='pr-1' />
                      <Button onClick={() => admCajauser.agregarRelacion('usuId','facUsuarioUsuId','http://serviciofact.mercelab.com/cajausuario/crear','http://serviciofact.mercelab.com/cajausuario/usuario?idcaja=','cjuId')} id='addusuario' icon="pi pi-arrow-circle-down" aria-label="Filter" severity='warning' />
                    </div>
                    <DataTable value={admCajauser.cajaDato} className="w-auto">
                      <Column field="usuNombrecompleto" header="Usuarios"></Column>
                      <Column body={(rowData) => admCajauser.accionCaja(rowData, 'cjuId', 'http://serviciofact.mercelab.com/cajausuario/','http://serviciofact.mercelab.com/cajausuario/usuario?idcaja=')} />
                    </DataTable>
                  </Card>
                </div>
                <div className='w-full m-2'>
                  <Card title="Serie">
                    ...
                    <Toast ref={admCajaser.toast} />
                      <AutoComplete
                        ...
                        completeMethod={(event) => cajaSerB.buscarC(`http://serviciofact.mercelab.com/cajaserie/notserie?idcaja=`, event, 'serSerie')}
                        onChange={(e) => admCajaser.setSelectedDato(e.value)}
                        dropdown className='pr-1' />
                      <Button onClick={() => admCajaser.agregarRelacion('serId','facSerieSerId','http://serviciofact.mercelab.com/cajaserie/crear','http://serviciofact.mercelab.com/cajaserie/serie?idcaja=', 'cjsId')} id='addusuario' icon="pi pi-arrow-circle-down" aria-label="Filter" severity='warning' />
                    </div>
                    <DataTable value={admCajaser.cajaDato}>
                      <Column field="serSerie" header="Serie"></Column>
                      <Column body={(rowData) => admCajaser.accionCaja(rowData, 'cjsId', 'http://serviciofact.mercelab.com/cajaserie/','http://serviciofact.mercelab.com/cajaserie/serie?idcaja=')} />
                    </DataTable>
                  </Card>
                </div>
                <div className='w-full m-2'>
                  <Card title="Tipo Documento">
                    ...
                    <Toast ref={admCajadoc.toast} />
                      <AutoComplete
                        ...
                        completeMethod={(event) => cajaTdocB.buscarC(`http://serviciofact.mercelab.com/cajadocumento/notdocumento?idcaja=`, event, 'tpdDescripcion')}
                        onChange={(e) => admCajadoc.setSelectedDato(e.value)}
                        dropdown className='pr-1' />
                      <Button onClick={() => admCajadoc.agregarRelacion('tpdId','facTipoDocumentoTpdId','http://serviciofact.mercelab.com/cajadocumento/crear','http://serviciofact.mercelab.com/cajadocumento/documento?idcaja=', 'cjdId')} id='addusuario' icon="pi pi-arrow-circle-down" aria-label="Filter" severity='warning' />
                    </div>
                    <DataTable value={admCajadoc.cajaDato}>
                      <Column field="tpdDescripcion" header="Documento" ></Column>
                      <Column body={(rowData) => admCajadoc.accionCaja(rowData, 'cjdId', 'http://serviciofact.mercelab.com/cajadocumento/','http://serviciofact.mercelab.com/cajadocumento/documento?idcaja=')} />
                    </DataTable>
                  </Card>
                </div>
              </div>
